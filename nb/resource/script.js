...
let block = { a: 10, b: 20, c: 40 },
	...
	board, key, grid, visual, visMatch, aurMatch, stateV, stateA, N, seq, delay, wl, lang
...
const audio = new AudioContext(),
	...
	main = () => {
		...
		const clear = () => {
			...
			help = (el, cls) => {
				...
				for (const txt of ["Start task on the circle", "Note squares" + (sound && dual ? " and audio" : ""),
					"Match " + N + " state" + (N > 1 ? "s" : "") + " back", "End task on the center cross"]) create("li", el.firstChild).textContent = txt
			},
			fscreen = () => {
				if (document.fullscreenElement) document.exitFullscreen()
				else board.parentNode.requestFullscreen()
			}
		[btn.right, btn.wrong] = [0, 0]
	...
	btn = (match, but) => {
		if (btn.fired[but]) return
		if (match) ++btn.right
		else ++btn.wrong
		btn.fired[but] = !0
	},
	run = () => {
		if (--run.n === -1) { clearSeq(); return result() }
		btn.fired = { eye: !1, ear: !1 }
		const vis = stateV[run.n],
			aur = stateA[run.n],
			el = $(vis)
		visMatch = vis === stateV[run.n + N]
		aurMatch = aur === stateA[run.n + N]
...
addEventListener("DOMContentLoaded", () => {
	[board, N, visual, key] = [document.body, range.start, [], Object.keys(block)[0]]
