...
const audio = new AudioContext(),
	...
	main = () => {
		...
		const capFirst = str => str.charAt(0).toUpperCase() + str.slice(1),
			...
			setting = prssd => {
				if (prssd) { mode = Object.keys(time).find(k => k !== mode); clear() }
		...
		board.className = ""
		ctrl("p", ["setting", "alt", "help"], [setting(), capFirst(opt), "?"], [el => el.textContent = setting(!0), el => el.textContent = alt(), el => help(el)])
